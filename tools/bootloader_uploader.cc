...
void setNodeId(uint8_t id)
{
    node_id = id;
    std::stringstream ss;
    ss << "Node ID set to: 0x" << std::hex << std::setw(2) << std::setfill('0')
       << (int)node_id;
    set_status(ss.str());
...
void rx_callback(struct can_frame rx_frame)
{
    ...
    if (cmd == 0x12 && rx_frame.can_dlc >= 4) {
        received_crc = (rx_frame.data[0] << 24) | (rx_frame.data[1] << 16) | (rx_frame.data[2] << 8) | rx_frame.data[3];
        crc_received = true;
        if (verbose_logging) {
            std::stringstream ss;
    ...
    if (verbose_logging) {
        std::stringstream ss;
        ss << "Response from node 0x" << std::hex << (int)nodeId << ", cmd=0x"
           << (int)cmd << ", DLC=" << std::dec << (int)rx_frame.can_dlc;
        add_log_message(ss.str());
    }
}
...
{
    if (timeout_ms == 0)
        timeout_ms = 10000; // Default if 0 passed
...
{
    if (timeout_ms == 0)
        timeout_ms = 1000; // Default if 0 passed
                 ...
                 bool show_status)
{
    ...
    if (show_status && verbose_logging) {
        ...
        std::stringstream ss;
        if (data.empty()) {
            ss << "Sent: " << cmd_desc << " to node 0x" << std::hex
               << (int)node_id;
        } else {
            if (cmd == 0x03) {
                ss << "Sent: " << cmd_desc << " to node 0x" << std::hex
                ...
                ss << "Sent: " << cmd_desc << " to node 0x" << std::hex
                   << (int)node_id << ", Data length: " << std::dec
                   << data.size();
            }
        }
        add_log_message(ss.str());
...
void init_ncurses()
{
    ...
    win_status = newwin(WIN_STATUS_HEIGHT, max_x,
                        WIN_MAIN_HEIGHT + WIN_PROGRESS_HEIGHT, 0);
    win_log =
        newwin(WIN_LOG_HEIGHT, max_x,
               WIN_MAIN_HEIGHT + WIN_PROGRESS_HEIGHT + WIN_STATUS_HEIGHT, 0);
    win_input = newwin(WIN_INPUT_HEIGHT, max_x,
                       WIN_MAIN_HEIGHT + WIN_PROGRESS_HEIGHT + WIN_STATUS_HEIGHT + WIN_LOG_HEIGHT,
                       0);
...
void draw_menu()
{
    const char *menu_items[] = {"Erase Flash", "Upload Firmware",
                                "Verify CRC", "Set Node ID",
                                "Device Info", "Exit Program"};

    int start_y = 4;
    for (int i = 0; i < 6; i++) {
        if (i == selected_menu) {
            wattron(win_main, COLOR_PAIR(COLOR_PAIR_SELECTED) | A_BOLD);
            mvwprintw(win_main, start_y + i * 2, 4, ">> %-20s <<",
                      menu_items[i]);
...
void show_device_info()
{
    ...
    if (sendCommand(0x05, {}, true)) {
        ...
        if (received_crc != 0xFFFFFFFF) {
            ...
            mvwprintw(win_main, 14, 4,
                      "Application: INVALID or not programmed");
        }
    }
...
bool write_bin_file(const std::string &filename)
{
    ...
    std::string confirm =
        get_user_input("Proceed with firmware upload? (y/n): ");
    ...
    while (running) {
        ...
        case KEY_ENTER:
            ...
            case 1: { // Upload firmware
                std::string filename =
                    get_user_input("Enter firmware file path: ");
                if (!filename.empty()) {
            ...
            case 3: { // Set node ID
                std::string id_str =
                    get_user_input("Enter node ID (hex, e.g., 0x01): ");
                if (!id_str.empty()) {
                    try {
                        uint8_t new_id;
                        ...
                        if (new_id <= 0x1F) {
                            ...
                            set_error(
                                "Node ID must be between 0 and 0x1F");
                        }
